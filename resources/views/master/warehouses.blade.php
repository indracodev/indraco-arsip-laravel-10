...
<div class="space-y-8" x-data="{ openAddWarehouse: false, openAddLocation: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex gap-2">
            <button @click="openAddWarehouse = true" type="button" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800 dark:bg-slate-800 hover:bg-slate-700 text-white font-bold text-xs transition">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
            <div class="flex items-start justify-between">
                <div>
                    <span class="px-2.5 py-1 rounded-md text-xs font-mono font-extrabold bg-amber-500/10 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300 border border-amber-500/30">
                        {{ $wh->code }}
                    </span>
                    <h2 class="text-lg font-extrabold text-slate-900 dark:text-white mt-2">{{ $wh->name }}</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 flex items-center gap-1 font-medium">
                        <i data-lucide="map-pin" class="w-3.5 h-3.5 text-slate-400"></i> {{ $wh->address ?? 'Alamat belum diisi' }}
                    </p>
                </div>
            </div>

            <div class="border-t border-slate-200 dark:border-slate-800 pt-4 space-y-3">
                <span class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Daftar Slot Rak & Baris:</span>
                ...
                <div class="space-y-2">
                    ...
                    <div class="p-3 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs">
                        <div>
                            <span class="font-extrabold text-emerald-700 dark:text-emerald-400 block">{{ $loc->full_location }}</span>
                            <span class="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Kapasitas Maksimal: {{ $loc->box_capacity }} Box</span>
                        </div>

                        <div class="flex items-center gap-3">
                            <span class="px-2.5 py-1 rounded-full bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-300 font-bold border border-slate-200 dark:border-slate-800">
                            ...
                            <form action="{{ route('master.warehouses.locations.destroy', $loc) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Hapus lokasi rak ini?')" class="p-1 text-slate-400 hover:text-rose-600 transition">
                    ...
                    @empty
                    <div class="p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl text-center text-xs text-slate-500 font-medium">
        ...
        <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form action="{{ route('master.warehouses.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" required uppercase placeholder="GUDANG-C" class="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl text-sm font-mono text-amber-600 dark:text-amber-400 uppercase font-bold focus:outline-none focus:border-amber-500">
                </div>
                <div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-amber-500 text-slate-950 text-xs font-black rounded-xl">Simpan Gudang</button>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form action="{{ route('master.warehouses.locations.store') }}" method="POST" class="space-y-4">
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-xs font-black rounded-xl">Tambah Rak</button>
                </div>
            </form>
        </div>
    </div>
