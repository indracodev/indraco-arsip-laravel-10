            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60 text-sm">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition">
                        <td class="py-4 px-4 font-mono text-xs text-amber-600 dark:text-amber-400 font-black">
                            {{ $dept->code }}
                        </td>
                        <td class="py-4 px-4 font-bold text-slate-900 dark:text-white">
                            {{ $dept->name }}
                        </td>
                        <td class="py-4 px-4 text-xs text-slate-600 dark:text-slate-300 font-medium">
                            {{ $dept->description ?? '-' }}
                        </td>
                        <td class="py-4 px-4 text-xs font-bold text-purple-700 dark:text-purple-300">
                            {{ $dept->archives_count }} Box/Berkas
                        </td>
                        <td class="py-4 px-4 text-right flex items-center justify-end gap-2">
                            <button @click="editItem = {{ json_encode($dept) }}" class="p-1.5 text-slate-500 hover:text-amber-600 dark:text-slate-400 dark:hover:text-amber-400 hover:bg-slate-100 dark:hover:bg-slate-900 rounded-lg transition">
                                <i data-lucide="edit" class="w-4 h-4"></i>
                            </button>
                            <form action="{{ route('master.departments.destroy', $dept) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Hapus departemen ini?')" class="p-1.5 text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-900 rounded-lg transition">
        ...
        <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form action="{{ route('master.departments.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" required uppercase maxlength="10" placeholder="FIN" class="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl text-sm font-mono text-amber-600 dark:text-amber-400 uppercase font-bold focus:outline-none focus:border-amber-500">
                </div>
                <div>
                    ...
                    <input type="text" name="name" required placeholder="Keuangan & Akuntansi" class="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl text-sm text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 font-medium">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-400 mb-1">Deskripsi</label>
                    <textarea name="description" rows="2" class="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-amber-500 font-medium"></textarea>
                </div>
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="px-4 py-2 bg-purple-600 text-white text-xs font-black rounded-xl">Simpan</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <div class="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
                ...
                <form :action="'{{ url('/master/departments') }}/' + editItem.id" method="POST" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-amber-500 text-slate-950 text-xs font-black rounded-xl">Update</button>
                    </div>
                </form>
            </div>
        </div>
