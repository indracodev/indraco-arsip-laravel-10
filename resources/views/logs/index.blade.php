    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2">
                <i data-lucide="history" class="w-7 h-7 text-cyan-400"></i>
                Global Log Audit Trail System
            </h1>
            <p class="text-slate-400 text-sm">Pencatatan riwayat otomatis untuk aktivitas penerimaan fisik gudang, peminjaman, dan pemusnahan dokumen.</p>
        </div>
    </div>

    <!-- Navigation Tabs & Filter Bar -->
    <div class="bg-slate-950/80 border border-slate-800 rounded-3xl p-5 shadow-xl space-y-4">
        <!-- Tabs -->
        <div class="flex border-b border-slate-800 gap-2">
            <a href="{{ route('logs.index', ['tab' => 'entry']) }}" class="px-4 py-2.5 text-xs font-bold rounded-t-xl transition flex items-center gap-2 {{ $tab === 'entry' ? 'bg-cyan-500/10 text-cyan-400 border-b-2 border-cyan-400' : 'text-slate-400 hover:text-white' }}">
                <i data-lucide="log-in" class="w-4 h-4"></i> Log Masuk Gudang
            </a>

            <a href="{{ route('logs.index', ['tab' => 'borrowing']) }}" class="px-4 py-2.5 text-xs font-bold rounded-t-xl transition flex items-center gap-2 {{ $tab === 'borrowing' ? 'bg-purple-500/10 text-purple-400 border-b-2 border-purple-400' : 'text-slate-400 hover:text-white' }}">
                <i data-lucide="file-symlink" class="w-4 h-4"></i> Log Peminjaman
            </a>

            <a href="{{ route('logs.index', ['tab' => 'destruction']) }}" class="px-4 py-2.5 text-xs font-bold rounded-t-xl transition flex items-center gap-2 {{ $tab === 'destruction' ? 'bg-rose-500/10 text-rose-400 border-b-2 border-rose-400' : 'text-slate-400 hover:text-white' }}">
        ...
        <form action="{{ route('logs.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
            ...
            <div>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Judul Arsip / No. Box / No. BAP..." class="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500">
            </div>

            <div>
                <select name="department_id" class="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-amber-500">
                    ...
                    <tbody class="divide-y divide-slate-800/60 text-xs">
                        ...
                        <tr class="hover:bg-slate-900/50 transition">
                            ...
                            <td class="py-3.5 px-4">
                                <a href="{{ route('archives.show', $eLog->archive) }}" class="font-bold text-white hover:text-amber-400 transition block">
                                    {{ $eLog->archive->title }}
                                </a>
                                <span class="text-slate-400 text-[10px]">{{ $eLog->archive->department->name ?? '' }}</span>
                            </td>
                    ...
                    <tbody class="divide-y divide-slate-800/60 text-xs">
                        ...
                        <tr class="hover:bg-slate-900/50 transition">
                            <td class="py-3.5 px-4 space-y-0.5">
                                <span class="text-purple-300 font-semibold block">Req: {{ \Carbon\Carbon::parse($bLog->request_date)->format('d M Y') }}</span>
                                <span class="text-slate-400 block">Est: {{ \Carbon\Carbon::parse($bLog->expected_return_date)->format('d M Y') }}</span>
                            </td>
                            <td class="py-3.5 px-4">
                                <span class="font-mono font-bold text-amber-400 text-[11px] block">{{ $bLog->archive->box_number }}</span>
                                <span class="font-bold text-white">{{ $bLog->archive->title }}</span>
                            </td>
                            <td class="py-3.5 px-4 text-slate-200 font-semibold">
                                {{ $bLog->borrower->name }}
                            </td>
                            ...
                            <td class="py-3.5 px-4">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-slate-900 border border-slate-800 text-purple-300">
                                    {{ $bLog->status }}
                                </span>
                            </td>
                            <td class="py-3.5 px-4 text-slate-400">
                    ...
                    <tbody class="divide-y divide-slate-800/60 text-xs">
                        ...
                        <tr class="hover:bg-slate-900/50 transition">
                            ...
                            <td class="py-3.5 px-4">
                                ...
                                <span class="text-slate-400 text-[10px]">{{ $dLog->archive->box_number }}</span>
                            </td>
                            <td class="py-3.5 px-4 text-rose-400 font-semibold">
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <a href="{{ route('destructions.bap', $dLog) }}" target="_blank" class="px-2.5 py-1 bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded font-bold hover:bg-amber-500/30">
                                    Cetak BAP
                                </a>
                            </td>
                        </tr>
